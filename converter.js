...
import { createPdfTabController } from './modules/tabs/pdf-tab.js?v=20260803a';
import {
    getDataUrlSize,
    getImageFormat,
...
import { createElements } from './modules/app-elements.js?v=20260803a';
import { createState } from './modules/app-state.js?v=20260803a';
...
async function loadTabPartials() {
    ...
    await Promise.all(tabs.map(async (name) => {
        const res = await fetch(`modules/tabs/html/tab-${name}.html?v=20260803a`);
